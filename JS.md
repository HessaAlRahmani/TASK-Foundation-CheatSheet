...
function foo(dish, price){
  return `${dish} is ${price} KD` ;
}
...
const foo = (dish, price) => `${dish} is ${price} KD`;
const foo = (x, y) => { return `${dish} is ${price} KD`};
const foo = () => "nothing";
```
...
class ClassName {
  ...
  // the arrow function way
  Concat = () => console.log(name + " is" + age) ;
}
```
